...
public final class PowerManagerService extends SystemService
    ...
    //add by jinlibo for lockscreen
    private static final int MSG_WAIT_BRIGHT_TIMEOUT = 4;
    ...
    // Dirty bit:wait brightness mode changed
    private static final int DIRTY_WAIT_BRIGHT_MODE = 1 << 15;
    ...
    //add by jinlibo for lockscreen
    private boolean mBrightnessWaitModeEnabled;
    //The wait brightness timeout
    ...
    private void enableBrightnessWaitLocked(){
        if(!mBrightnessWaitModeEnabled){
            mBrightnessWaitModeEnabled = true;
            mDirty |= DIRTY_WAIT_BRIGHT_MODE;
            
            Message msg = mHandler.obtainMessage(MSG_WAIT_BRIGHT_TIMEOUT);
            msg.setAsynchronous(true);
            mHandler.sendMessageDelayed(msg, mWaitBrightTimeout);
        }
    }
    
    private void disableBrightnessWaitLocked(){
        if(mBrightnessWaitModeEnabled){
            mHandler.removeMessages(MSG_WAIT_BRIGHT_TIMEOUT);
            mBrightnessWaitModeEnabled  = false;
            
            mDirty |= DIRTY_WAIT_BRIGHT_MODE;
        }
    }
    
    private void startWakeUpReadyInternal(long eventTime, int uid){
        synchronized(mLock){
            if(DEBUG){
                Slog.d(TAG, "startWakeUpReadyInternal, mBrightnessWaitModeEnabled = "+
                        mBrightnessWaitModeEnabled);
            }
            
            if(!mBrightnessWaitModeEnabled){
                if(wakeUpNoUpdateWithoutInteractiveLocked(eventTime,uid)){
    ...
    private void resetWaitBrightTimeoutLocked(){
        mHandler.removeMessages(MSG_WAIT_BRIGHT_TIMEOUT);
    ...
    private void stopWakeUpReadyInternal(long eventTime, int uid, boolean enableBright){
        synchronized(mLock){
            if(DEBUG){
                Slog.d(TAG, "stopWakeUpReadyInternal, mBrightnessWaitModeEnabled = "+
                        mBrightnessWaitModeEnabled + " enableBright = "+ enableBright);
            }
            
            if(mBrightnessWaitModeEnabled){
                if(enableBright){
                    setWakefulnessLocked(WAKEFULNESS_AWAKE, 0);
                    disableBrightnessWaitLocked();
                    updatePowerStateLocked();
                }else{
                    goToSleepNoUpdateLocked(eventTime, PowerManager.GO_TO_SLEEP_REASON_APPLICATION,
                            0, uid);
                    updatePowerStateLocked();
                }
            }
    ...
    private boolean wakeUpNoUpdateLocked(long eventTime, int uid) {
        ...
        //modified by jinlibo for lockscreen
        if (eventTime < mLastSleepTime || (mWakefulness == WAKEFULNESS_AWAKE && !mBrightnessWaitModeEnabled)
                || !mBootCompleted || !mSystemReady || mProximityPositive) {
            return false;
        }
    ...
    //add by jinlibo for lockscreen
    private boolean wakeUpNoUpdateWithoutInteractiveLocked(long eventTime, int uid){
        if(DEBUG_SPEW){
            Slog.d(TAG, "wakeIpNoUpdateWithoutInteractiveLocked: eventTime="+
                    eventTime+", uid="+uid);
        }
        ...
        Trace.traceBegin(Trace.TRACE_TAG_POWER, "wakeIpNoUpdateWithoutInteractive");
        try{
            switch(mWakefulness){
                case WAKEFULNESS_ASLEEP:
                    Slog.i(TAG, "Waking up from sleep(uid "+uid+")...");
                    Jlog.d(JlogConstants.JLID_PMS_WAKEFULNESS_ASLEEP, "JLID_PMS_WAKEFULNESS_ASLEEP");
                    break;
                case WAKEFULNESS_DREAMING:
                    Slog.i(TAG, "Waking up from dream(uid "+uid+")");
                    Jlog.d(JlogConstants.JLID_PMS_WAKEFULNESS_DREAMING, "JLID_PMS_WAKEFULNESS_DREAMING");
                    break;
                case WAKEFULNESS_DOZING:
                    Slog.i(TAG, "Waking up from dozing(uid "+uid+")");
                    Jlog.d(JlogConstants.JLID_PMS_WAKEFULNESS_NAPPING, "JLID_PMS_WAKEFULNESS_NAPPING");
                    break;
            }
        ...
        try {
            switch (reason) {
                ...
                case PowerManager.GO_TO_SLEEP_REASON_PROXIMITY:
                    ...
                    break;
                case PowerManager.GO_TO_SLEEP_REASON_WAIT_BRIGHTNESS_TIMEOUT:
                    Slog.i(TAG, "Going to sleep due to wait bright timeout (uid " + uid +")...");
                    break;
                default:
            ...
            //modified by jinlibo for lockscreen
            if ((flags & PowerManager.GO_TO_SLEEP_FLAG_NO_DOZE) != 0 || mBrightnessWaitModeEnabled) {
                disableBrightnessWaitLocked();
    ...
    private void setWakefulnessLocked(int wakefulness, int reason) {
        //modified by jinlibo for lockscreen
        if (mWakefulness != wakefulness || mBrightnessWaitModeEnabled) {
    ...
    //add by jinlibo for lockscreen
    private void handleWaitBrightTimeout(){
        //runs on handler thread
        synchronized(mLock){
            if(DEBUG){
                Slog.d(TAG, "handleWaitBrightTimeout mBrightnessWaitModeEnabled="+
                        mBrightnessWaitModeEnabled + "mWakefulness="+mWakefulness);
            }
            
            if(mBrightnessWaitModeEnabled){
                goToSleepNoUpdateLocked(SystemClock.uptimeMillis(), 
                        PowerManager.GO_TO_SLEEP_REASON_WAIT_BRIGHTNESS_TIMEOUT, 0, Process.SYSTEM_UID);
                updatePowerStateLocked();
            }
        }
    }
    ...
    private boolean updateDisplayPowerStateLocked(int dirty) {
        ...
        if ((dirty & (DIRTY_WAKE_LOCKS | DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS
                ...
                | DIRTY_SETTINGS | DIRTY_SCREEN_BRIGHTNESS_BOOST | DIRTY_BOOT_IPO
                | DIRTY_WAIT_BRIGHT_MODE)) != 0) {
            ...
            //add by jinlibo for lockscreen
            mDisplayPowerRequest.brightnessWaitMode = mBrightnessWaitModeEnabled;
            ...
            if (DEBUG_SPEW) {
                Slog.d(TAG, "updateDisplayPowerStateLocked: mDisplayReady=" + mDisplayReady
                        ...
                        + ", mRequestWaitForNegativeProximity=" + mRequestWaitForNegativeProximity
                        + ", mBrightnessWaitModeEnabled="+ mBrightnessWaitModeEnabled);
            }
        }
    ...
    private boolean isInteractiveInternal() {
        synchronized (mLock) {
            //modified by jinlibo for lockscreen 
            return PowerManagerInternal.isInteractive(mWakefulness) && !mBrightnessWaitModeEnabled;
        }
    }
    ...
    public static void lowLevelShutdown() {
        Slog.d(TAG, "before SystemProperties set shutdown"); ///gepengfei add log4
        SystemProperties.set("sys.powerctl", "shutdown");
        Slog.d(TAG, "after SystemProperties set shutdown"); ///gepengfei add log5
    }
    ...
    public static void lowLevelReboot(String reason) {
        ...
        if (reason.equals(PowerManager.REBOOT_RECOVERY)) {
            ...
            Slog.d(TAG, "before SystemProperties set reboot"); // gepengfei add log1
            SystemProperties.set("sys.powerctl", "reboot," + reason);
            Slog.d(TAG, "after SystemProperties set reboot"); // gepengfei add log2
            duration = 20 * 1000L;
        }
        try {
	...
	Slog.d(TAG, "lowLevelReboot out"); // gepengfei add log3
    }
    ...
    private void dumpInternal(PrintWriter pw) {
        ...
        synchronized (mLock) {
            ...
            pw.println("  mBrightnessWaitModeEnabled="
                    + mBrightnessWaitModeEnabled);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SCREEN_BRIGHTNESS_BOOST_TIMEOUT:
                    ...
                    break;
                case MSG_WAIT_BRIGHT_TIMEOUT:
                    //add by jinlibo for lockscreen
                    handleWaitBrightTimeout();
                    break;
            }
        }
    }
    ...
    private final class BinderService extends IPowerManager.Stub {
        ...
        //fengyaling add begin
        public void startWakeUpReady(final long eventTime){
            ...
            try{
                startWakeUpReadyInternal(eventTime, uid);
            }finally{
        ...
        public void stopWakeUpReady(long eventTime,boolean enableBright){
            ...
            final long ident = Binder.clearCallingIdentity();
            try{
                stopWakeUpReadyInternal(eventTime, uid, enableBright);
            }finally{
                Binder.restoreCallingIdentity(ident);
            } 
         }
