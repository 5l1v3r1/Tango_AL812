...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    // settings.
    private static final int DATABASE_VERSION = 119;//modify by sunsongjian [HQ01864688]
...
if(!(SystemProperties.get("ro.hw_cl_email").equals("1"))){
	...
	//add by sunsongjian 20160421 begin
    @Override
    public void onDowngrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        //if (oldVersion == 119 && newVersion == 118) {
			Log.w(TAG, "onDowngrade settings database from oldVersion " + oldVersion + " to " + newVersion);
        //} else {
	...
	//add by sunsongjian [HQ01864688] begin
        if (upgradeVersion < 119) {
            //if (mUserHandle == UserHandle.USER_OWNER) {
            	Log.w(TAG, "Upgrading settings database ENHANCED_4G_MODE_ENABLED --> off ");
                db.beginTransaction();
                SQLiteStatement stmt = null;
                try {
				    stmt = db.compileStatement("UPDATE global SET value = ? "
														+ " WHERE name = ?");
                    stmt.bindString(1, "0"); // new value
                    stmt.bindString(2, Settings.Global.ENHANCED_4G_MODE_ENABLED);	
                    stmt.execute();
    ...
    private void loadVibrateWhenRingingSetting(SQLiteDatabase db) {
        ...
        try {
            stmt = db.compileStatement("INSERT OR IGNORE INTO system(name,value)"
                    + " VALUES(?,?);");
            loadSetting(stmt, Settings.System.VIBRATE_WHEN_RINGING, 1);         //hanchao change HQ01432249 20151008
        } finally {
            if (stmt != null) stmt.close();
        }
    }
    ...
    private void loadSystemSettings(SQLiteDatabase db) {
	 ...
	 //add by xuweijie end
			...
			/*add by wangjing3  20151004 HQHQ01393239*/
	 		//niyangdel loadSetting(stmt, "photo_backsize_cust", new String("4160x2336"));
	 		/*add end*/
    ...
    private void loadGlobalSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            loadBooleanSetting(stmt, Settings.Global.GUEST_USER_ENABLED,
                    R.bool.def_guest_user_enabled);
			//modify  by sunsongjian 20160323 for default enhanced 4g lte close
