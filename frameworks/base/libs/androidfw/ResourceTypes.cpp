...
//modified by WengRongjian for auto language config from ProjectConfig.mk 
    ...
    size_t cSize = sizeof(countryCode) / sizeof(countryCode[0]);
...
//#ifdef HQ_COUNTRYCODE_CUST //added by wangyi.2015.7.27
		...
		)
		{
			int bIsContinue = 1;
#if 0
			for (size_t nIndex =0; nIndex < cSize; cSize)
			{
				if(0 == strlen(countryCode[nIndex]))
				{
					continue;
				}

				if (0 == strcmp(locale, (*locales)[nIndex].string()))
				{
					bIsContinue = 0;
					break;
				}
