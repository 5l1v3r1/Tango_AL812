...
public class CallerInfo {
    ...
    private static int getColumnIndexForPersonId(Uri contactRef, Cursor cursor) {
		...
		if (Build.TYPE.equals("eng")){//leiyuanhua 160714 add for red line HQ02026580
    ...
    private static String getGeoDescription(Context context, String number) {
        ...
        PhoneNumber pn = null;
        try {
            if (VDBG && android.os.Build.TYPE.equals("eng")) Rlog.v(TAG, "parsing '" + number
                            + "' for countryIso '" + countryIso + "'...");
            pn = util.parse(number, countryIso);
            if (VDBG && android.os.Build.TYPE.equals("eng")) Rlog.v(TAG, "- parsed number: " + pn);
