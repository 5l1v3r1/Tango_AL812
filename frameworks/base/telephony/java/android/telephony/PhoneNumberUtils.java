...
public class PhoneNumberUtils
{
	...
	soltId=SubscriptionManager.getSlotId(subId);
   ...
   if(SystemProperties.get("ro.hq.custom.ecc").equals("1")){
    ...
    public static boolean isEmergencyNumberExt(String number, int phoneType) {
       ...
       if (Build.TYPE.equals("eng")) {
           Log.i(LOG_TAG,"[isEmergencyNumberExt], number:" + number + ", phoneType:" + phoneType);
       }
        if (number == null) {
            return false;
        }
        ...
        if (PHONE_TYPE_CDMA == phoneType) {
            ...
            if(SystemProperties.get("ro.hq.custom.ecc").equals("1")){
                 Log.d(LOG_TAG, "[isEmergencyNumber] soltId: " + soltId);
		 String mccmnc = getSimMccMnc(soltId);
    ...
    //}     
    private static String getOperatorMccmnc() {
	...
	private static void initEccPatternAndKey(int slotId) {
                Log.i("huangshuo","[initEccPatternAndKey]:virtualKey="+virtualKey);
		Log.i("huangshuo","[initEccPatternAndKey]:virtualPattern="+virtualPattern);
		if(virtualKey!=null && virtualKey.length()>0  
			&& (virtualPattern==VIRTUAL_PATTERN_IMSI || virtualPattern==VIRTUAL_PATTERN_GID1
			|| virtualPattern==VIRTUAL_PATTERN_SPN || virtualPattern==VIRTUAL_PATTERN_EFFILE
			|| virtualPattern==PATTERN_MCCMNC)){
			return;
		}
		virtualPattern = PATTERN_MCCMNC;
		String mccmnc = getSimMccMnc(slotId);
		 Log.i("huangshuo","[initEccPatternAndKey]:mccmnc="+mccmnc);
		 Log.i("huangshuo","[initEccPatternAndKey]:hasVirtualKey="+EccOverride.getInstance().hasVirtualMccmnc(mccmnc));
		if(!EccOverride.getInstance().hasVirtualMccmnc(mccmnc)){
			...
			String imsi = getDefaultImsi(slotId);
			 Log.i("huangshuo","[initEccPatternAndKey]:imsi="+imsi);
			String imsipattern = EccOverride.getInstance().getVirtualKeyByImsi(imsi);
			 Log.i("huangshuo","[initEccPatternAndKey]:imsipattern="+imsipattern);
			if(imsipattern!=null){
				virtualPattern = VIRTUAL_PATTERN_IMSI;
				virtualKey = imsipattern;
				return;
			}
		}
			...
			String gid1 = getDefaultGID1(slotId);
		        Log.i("huangshuo","[initEccPatternAndKey]:gid1="+gid1);
			Log.i("huangshuo","[initEccPatternAndKey]:hasVirtualKey="+EccOverride.getInstance().hasVirtualKey(mccmnc+gid1));
			if(EccOverride.getInstance().hasVirtualKey(mccmnc+gid1)){
				virtualPattern = VIRTUAL_PATTERN_GID1;
				virtualKey = mccmnc+gid1;
				return;
			}	
		}
			...
			String efspn = getDefaultEfspn(slotId);
		        Log.i("huangshuo","[initEccPatternAndKey]:emccmnc+efspn="+mccmnc+efspn);
			Log.i("huangshuo","[initEccPatternAndKey]:hasVirtualKey="+EccOverride.getInstance().hasVirtualKey(mccmnc+efspn));
			if(EccOverride.getInstance().hasVirtualKey(mccmnc+efspn)){
				virtualPattern = VIRTUAL_PATTERN_SPN;
				virtualKey = mccmnc+efspn;
				return;
			}
	        }
			...
			Log.i("huangshuo","[initEccPatternAndKey]:efspn="+efsst);
			Log.i("huangshuo","[initEccPatternAndKey]:hasVirtualKey="+EccOverride.getInstance().hasVirtualKey(mccmnc+efsst));
			if(EccOverride.getInstance().hasVirtualKey(mccmnc+efsst)){
				virtualPattern = VIRTUAL_PATTERN_EFFILE;
				virtualKey = mccmnc+efsst;
				return;
			}
		}
	...
	public static String getDefaultGID1(int slotId) {
		...
		String tmpGID1 = "";
		if(slotId==0){
			tmpGID1 = SystemProperties.get("ril.gid1.sim1");
			Log.d(LOG_TAG,"GID1: "+ tmpGID1+" ,"+slotId);
		}else if(slotId==1){
			tmpGID1 = SystemProperties.get("ril.gid1.sim2");
			Log.d(LOG_TAG,"GID1: "+ tmpGID1+" ,"+slotId);
		}else{
			tmpGID1 = "";
		}	
	...
	private static String getDefaultEfspn(int slotId) {
		String tmpSpn = "";
		...
		Log.d("hhh","[getDefaultEfspn] slotId = " + slotId + ",	tmpSpn = "+ tmpSpn);
		
		return tmpSpn;
	}
	private static String getDefaultSST(int slotId) {
		String tmpSST = "";
		if(slotId==0){
