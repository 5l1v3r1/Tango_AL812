...
public class SubscriptionManager {
    ...
    /**
     * Get SubId by SubscriberId , imsi
     *
     * @param imsi.
     * @return SubId or  -1 when imsi not found.
     * @hide
    */
    public static int getSubIdUsingImsi(String imsi) {
        if (null == imsi) {
        ...
        try {
            ISub iSub = ISub.Stub.asInterface(ServiceManager.getService("isub"));
            if (iSub != null) {
               result = iSub.getSubIdUsingImsi(imsi);
            }
        } catch (RemoteException ex) {
            // ignore it
        }
    ...
    /**
     * add for c2k. get imsi for CDMA LTE.
     * @param subId subId
     * @hide
     */
    public static String getSubscriberIdForLteDcPhone(int subId) {
        if (!isValidSubscriptionId(subId)) {
        ...
        try {
            ISub iSub = ISub.Stub.asInterface(ServiceManager.getService("isub"));
            if (iSub != null) {
               result = iSub.getSubscriberIdForLteDcPhone(subId);
            }
        } catch (RemoteException ex) {
            // ignore it
        }
