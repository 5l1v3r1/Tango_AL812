...
public class SvlteModeController extends Handler {
    ...
    */
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case EVENT_CDMA_RADIO_CHANGE_FOR_AVALIABLE:
                if (mWaitC2KRadioAvailable) {
                    mWaitC2KRadioAvailable = false;
                    tryToSwitchMode();
                }
                break;
            case EVENT_C2K_WP_CARD_TYPE_READY:
                mWaitC2KRadioAvailable = true;
                tryToSwitchMode();
                break;
    ...
    private boolean mWaitC2KRadioAvailable = false;
    ...
    private void tryToSwitchMode() {
        ...
        logicLog("tryToSwitchMode, RadioState: " + rs
            + " mWaitC2KRadioAvailable: " + mWaitC2KRadioAvailable
            + " mAlreadyRegistForRadioState: " + mAlreadyRegistForRadioState);
        if (blockBySimSwitch()) {
            unregisterForRadioAvailable();
        } else if (mInSwitching.get() && mWaitC2KRadioAvailable) {
            logicLog("[tryToSwitchMode] Switching now, pended");
            unregisterForRadioAvailable();
            mPendingCardTypes = Arrays.copyOf(UiccController.getInstance().getC2KWPCardType(),
            TelephonyManager.getDefault().getPhoneCount());
        } else if (rs != RadioState.RADIO_UNAVAILABLE) {
            unregisterForRadioAvailable();
            onC2kCardTypeReady();
        } else {
            mWaitC2KRadioAvailable = true;
            if (!mAlreadyRegistForRadioState) {
                ...
                ((SvltePhoneProxy) PhoneFactory.getPhone(sCdmaSocketSlotId))
                        .getNLtePhone().mCi.registerForAvailable(this,
                        EVENT_CDMA_RADIO_CHANGE_FOR_AVALIABLE, null);
            }
        }
    }

    private void unregisterForRadioAvailable() {
        mWaitC2KRadioAvailable = false;
        if (mAlreadyRegistForRadioState) {
            ((SvltePhoneProxy) PhoneFactory.getPhone(getCdmaSocketSlotId()))
                    .getNLtePhone().mCi.unregisterForAvailable(this);
    ...
    private int[] calculateCardMode() {
        ...
        if (slot0Prefer == slot1Prefer && slot1Prefer == RADIO_TECH_MODE_SVLTE) {
            ...
            if (cardModes[0] == RADIO_TECH_MODE_CSFB && cardModes[1] == RADIO_TECH_MODE_CSFB
                    && (mCardTypes[0] == UiccController.CARD_TYPE_NONE
                    || mCardTypes[1] == UiccController.CARD_TYPE_NONE)) {
                if (mCardTypes[0] == UiccController.CARD_TYPE_NONE
                        && mCardTypes[1] == UiccController.CARD_TYPE_NONE) {
                    cardModes[sCdmaSocketSlotId] = RADIO_TECH_MODE_SVLTE;
                } else {
                    if (mCardTypes[sCdmaSocketSlotId] == UiccController.CARD_TYPE_NONE) {
                        cardModes[sCdmaSocketSlotId] = RADIO_TECH_MODE_SVLTE;
                    }
                }
            }
