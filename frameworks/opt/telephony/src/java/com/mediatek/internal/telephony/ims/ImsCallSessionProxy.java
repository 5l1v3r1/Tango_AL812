...
public class ImsCallSessionProxy extends IImsCallSession.Stub {
    ...
    public void merge() {    
        ...
        if (myCallInfo.mIsConference == false && beMergedCallInfo.mIsConference == false) {
            ...
            if (myCallInfo.mIsConference && myCallInfo.mState == ImsCallInfo.State.HOLDING) {
                ...
                result = mHandler.obtainMessage(EVENT_ADD_CONFERENCE_RESULT);
                mImsRILAdapter.inviteParticipantsByCallId(Integer.parseInt(beMergedCallInfo.mCallId),
    ...
    private class MyHandler extends Handler {
        ...
        /// M: ALPS02492264, RFC 3261: pau might be [display-name] LAQUOT addr-spec RAQUOT
        /// Showing priority: display-name > addr-spec. 
        /// eg: ""4101" <sip:666662400@ims.mnc001.mcc214.3gppnetwork.org>"
        /// UI should show 4101. @{
        ...
        private void updatePau(String pau) {
            Rlog.d(LOG_TAG, "pau: " + pau);
            String sipNumber = getFieldValueFromPau(pau, PAU_SIP_NUMBER_FIELD);
            Rlog.d(LOG_TAG, "getSipNumberFromPAU()... sipNumber: " + sipNumber);
        ...
        private int sipCauseFromCode(int causeCode) {
            ...
            switch (causeCode) {
			    ...
			    /// M: for ALPS02581949. @{
                case CallFailCause.FDN_BLOCKED:
                    return ImsReasonInfo.CODE_LOCAL_FDN_BLOCKED;
                /// @}
                    ...
                    if (mIsMerging && (!callInfo[0].equals(mCallId))) {
                        ...
                        switch (msgType) {
                            case 0:
                                ...
                                /// M: ALPS02554677, get display name from PAU for MT. @{
                                if ((callInfo[8] != null) && (!callInfo[8].equals(""))) {
                                    String displayName = getDisplayNameFromPau(callInfo[8]);
                                    Rlog.d(LOG_TAG, "get display name: "+ displayName +
                                                          " from PAU for MT: " + callInfo[8]);
                                    if (displayName.trim().length() > 0) {
                                        mCallProfile.setCallExtra(
                                                   ImsCallProfile.EXTRA_CNA, displayName);
