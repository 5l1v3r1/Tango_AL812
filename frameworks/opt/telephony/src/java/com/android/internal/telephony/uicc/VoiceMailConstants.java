...
class VoiceMailConstants {
	...
	private HashMap<String, String[]> CarrierVirtualVmMap;//add by zhaizhanfeng for virtual voicemail at150811

    static final String LOG_TAG = "VoiceMailConstants";
    static final String PARTNER_VOICEMAIL_PATH ="etc/voicemail-conf.xml";
	static final String PARTNER_VIRTUALVOICEMAIL_PATH ="etc/virtualNets-conf.xml";//add by zhaizhanfeng for virtual voicemail at150811
   ...
   //add by zhaizhanfeng for virtual voicemail at150811 end
    VoiceMailConstants () {
	   ...
	   //add by zhaizhanfeng for virtual voicemail at150811 start 
		CarrierVirtualVmMap = new HashMap<String, String[]>();
    ...
    private void loadVoiceMail() {
        ...
        //add by zhaizhanfeng for virtual voicemail at150811 start 
		final File virtualvmFile = new File(Environment.getRootDirectory(),
                PARTNER_VIRTUALVOICEMAIL_PATH);
		//add by zhaizhanfeng for virtual voicemail at150811 end
        try {
		...
		//add by zhaizhanfeng for virtual voicemail at150811 start
		try {
            vmReader = new FileReader(virtualvmFile);
        } catch (FileNotFoundException e) {
            Rlog.w(LOG_TAG, "Can't open " +
        ...
        try {
            ...
            while (true) {
                ...
                String gid1 = parser.getAttributeValue(null, "gid1");
                String sst = parser.getAttributeValue(null, "sst");
				...
				if (!TextUtils.isEmpty(gid1)) {
                    isVritual = true;
                    CarrierVirtualVmMapByGid1.put(numeric + gid1, data);
                }
                if (!TextUtils.isEmpty(sst)) {
                    isVritual = true;
                    CarrierVirtualVmMapBySst.put(numeric + sst, data);
                }
                if (!isVritual) {
                    CarrierVirtualVmMap.put(numeric, data);
                }
				//add by zhaizhanfeng for virtual voicemail at150811 end
            }
