...
// M: MTK added end
    ...
    protected void updateSpnDisplay(boolean forceUpdate) {
        ...
        if (showPlmn != mCurShowPlmn
            ...
            if (isInSvlteMode()) {
                ...
                phoneId = mSvlteSST.getPhoneId();
                log("use SvlteSST phone id:" + phoneId);
            }
            ...
            if (SvlteUtils.getSlotId(phoneId) == 0) {
                //modify by yanqing for HQ01364279 HQ01364280 HQ01364282 start
                ...
                //modify by yanqing for HQ01364279 HQ01364280 HQ01364282 end
                SubscriptionManager.putPhoneIdAndSubIdExtra(intent0, phoneId);
                mPhone.getContext().sendStickyBroadcastAsUser(intent0, UserHandle.ALL);

            } else if (SvlteUtils.getSlotId(phoneId) == 1){
                //modify by yanqing for HQ01364279 HQ01364280 HQ01364282 start
                ...
                //modify by yanqing for HQ01364279 HQ01364280 HQ01364282 end
                SubscriptionManager.putPhoneIdAndSubIdExtra(intent1, phoneId);
	...
	//add by huangshuo for HQ01791793 on 2016/03/22
	    int oldState=mSS.getRilVoiceRadioTechnology();
	     log("oldState"+oldState);
	    int newState=mNewSS.getRilVoiceRadioTechnology();
	     log("newState"+newState);
	    if(oldState==ServiceState.RIL_RADIO_TECHNOLOGY_LTE && newState==ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN){
		TelecomManager telecomm = (TelecomManager) mPhone.getContext().getSystemService(Context.TELECOM_SERVICE);
			if (telecomm == null || !telecomm.isInCall()) {
					Intent i =new Intent("huawei.intent.action.IMS_SERVICE_STATE_CHANGED");
				        i.putExtra("state","UNREGISTERED");
					mPhone.getContext().sendBroadcast(i);
				}
	    }
	   //add by huangshuo for HQ01829310 on 2016/03/28
	   try{
	   		boolean imsRegStatus = ImsManager.getInstance(mPhone.getContext(),
                        						mPhone.getPhoneId()).getImsRegInfo();
			log("getImsRegInfo(): " + imsRegStatus);
		    if(imsRegStatus && oldState==ServiceState.RIL_RADIO_TECHNOLOGY_UNKNOWN 
				&& newState==ServiceState.RIL_RADIO_TECHNOLOGY_LTE){
		 
				TelecomManager telecomm = (TelecomManager) mPhone.getContext().getSystemService(Context.TELECOM_SERVICE);
				if (telecomm == null || !telecomm.isInCall()) {
						Intent i =new Intent("huawei.intent.action.IMS_SERVICE_STATE_CHANGED");
					        i.putExtra("state","REGISTERED");
						mPhone.getContext().sendBroadcast(i);
				}
		    }
	   } catch (ImsException ex) {
          log("Fail to get Ims Status");
       }     
	   //end  by huangshuo for HQ01829310 on 2016/03/28
        }
