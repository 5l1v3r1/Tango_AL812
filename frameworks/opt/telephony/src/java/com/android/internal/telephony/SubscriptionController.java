...
/*HQ_xionghaifeng 20160831 add for HQ02048995 start*/
import com.android.internal.telephony.Phone;
import com.android.internal.telephony.PhoneFactory;
import com.android.internal.telephony.uicc.UiccController;
import com.android.internal.telephony.uicc.IccRecords;
/*HQ_xionghaifeng 20160831 add for HQ02048995 end*/
...
public class SubscriptionController extends ISub.Stub {
    ...
    /*HQ_xionghaifeng 20160831 add for HQ02048995 start*/
    public int getSubIdUsingImsi(String imsi) {
        if(TextUtils.isEmpty(imsi)) {
            logd("[getSubIdUsingImsi]- imsi is null ");
            return SubscriptionManager.INVALID_SUBSCRIPTION_ID;
        }
        int subId = SubscriptionManager.INVALID_SUBSCRIPTION_ID;     //invalid subid
        Phone[] phones = PhoneFactory.getPhones();
        for(Phone phone: phones) {
            if(TextUtils.isEmpty(phone.getSubscriberId())) {
                logd("[getSubIdUsingImsi] getSubscriberId is null");
                continue;
            }
            subId = getSubIdUsingPhoneId(phone.getPhoneId());
            // CDMA phone
            if(PhoneConstants.PHONE_TYPE_CDMA == phone.getPhoneType()) {
                logd("[getSubIdUsingImsi] subId = " + subId + ", imsi is:" + /*imsi*/"****");
                if(imsi.equals(phone.getSubscriberId())) {
                    logd("[getSubIdUsingImsi] CDMA 3G imsi is: " + /*phone.getSubscriberId()*/"***");
                    return subId;
                }
                //CDMA LTE IMSI
                if(!TextUtils.isEmpty(getSubscriberIdForLteDcPhone(subId))
                            && imsi.equals(getSubscriberIdForLteDcPhone(subId))) {
                    logd("[getSubIdUsingImsi] CDMA 4G imsi is: " + /*getSubscriberIdForLteDcPhone(subId)*/"***");
                    return subId;
                 }
            //not CDMA phone
            } else if(imsi.equals(phone.getSubscriberId())) {
                logd("[getSubIdUsingImsi] not CDMA imsi is: " + /*phone.getSubscriberId()*/"***");
                return subId;
            }
        }
        logd("[getSubIdUsingImsi] No such imsi return -1");
        return SubscriptionManager.INVALID_SUBSCRIPTION_ID; //No such imsi
    }

    public String getSubscriberIdForLteDcPhone(int subId) {
        if (!SubscriptionManager.isValidSubscriptionId(subId)) {
            logd("[getSubscriberIdForLteDcPhone]- invalid subId, subId =" + subId);
            return null;
        }
        int phoneId = getPhoneId(subId);
        String imsi4g = null;
        try {
            ...
            if (records != null) {
                imsi4g = records.getIMSI();
            } else {
                logd("[getSubscriberIdForLteDcPhone] 3gpp records is null");
            }
        } catch (RuntimeException e) {
            logd("[getSubscriberIdForLteDcPhone] UiccController is not ready");
        }
       return imsi4g;
   }
   /*HQ_xionghaifeng 20160831 add for HQ02048995 end*/
