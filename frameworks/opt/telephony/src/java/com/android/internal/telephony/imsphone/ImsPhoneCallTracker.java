...
public final class ImsPhoneCallTracker extends CallTracker {
    ...
    void callEndCleanupHandOverCallIfAny() {
        if (mHandoverCall.mConnections.size() > 0) {
			...
			//add by sunsongjian HQ01628224 begin
			try {
				boolean imsRegStatus = ImsManager.getInstance(mPhone.getContext(), mPhone.getPhoneId()).getImsRegInfo();
				log("Ims Registration Status = " + imsRegStatus);
				if (imsRegStatus) {
    ...
    private int getDisconnectCauseFromReasonInfo(ImsReasonInfo reasonInfo) {
        ...
        switch (code) {
			...
			/// M: for ALPS02581949. @{
			case ImsReasonInfo.CODE_LOCAL_FDN_BLOCKED:
