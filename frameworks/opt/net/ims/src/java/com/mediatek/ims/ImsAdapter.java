...
public class ImsAdapter extends BroadcastReceiver {
    ...
    ///M: ALPS02635149 optimizing socket connect sec. @{
    private static final int RETRY_MAX_COUNTSEC = 3000;
    private static final int RETRY_INTERNAL = 200;
    private static boolean misEnterCountSec = false;
    private static int mCurrentCountSec = 0;
    /// @}
        ...
        public void run() {
            ...
            if (misImsAdapterInit) {
                if (mIO.connectSocket() == true) {
                    ...
                    ///M: ALPS02635149 optimizing socket connect sec. @{
                    Xlog.d(TAG, "EnableImsRunnable(): misEnterCountSec:"+ misEnterCountSec);
                    if(mCurrentCountSec == RETRY_MAX_COUNTSEC) {
                        stopVaProcess();
                    }
                    /// @}
    ...
    private void enableImsAdapter2ndStage() {
        ...
        ///M: ALPS02635149 optimizing socket connect sec. @{
        if (misEnterCountSec && mCurrentCountSec <= (RETRY_MAX_COUNTSEC - RETRY_INTERNAL)) {
            mCurrentCountSec += RETRY_INTERNAL;
        } else {
            mCurrentCountSec = RETRY_MAX_COUNTSEC;
        }
        /// @}
        mEnableImsRunnable = new EnableImsRunnable();
        mHandler.postDelayed(mEnableImsRunnable, mCurrentCountSec);
    }
