...
public class TelecomUtils {
     ...
     */
    public static boolean startIpPrefixSetting(Context context, PhoneAccountHandle account,
            boolean isIpCall) {
        if (!isIpCall || account == null || "E".equalsIgnoreCase(account.getId())) {
            return false;
        }
        String ipPrefix = getIpPrefix(context, account);
        if (TextUtils.isEmpty(ipPrefix) && isRadioOn(account)) {
            Log.d(TAG, " sub id  = " + account.getId());
            final Intent intentSettings = new Intent(Intent.ACTION_MAIN);
            intentSettings.setClassName(Constants.PHONE_PACKAGE,
                    Constants.IP_PREFIX_SETTING_CLASS_NAME);
            intentSettings.putExtra(PhoneConstants.SUBSCRIPTION_KEY,
     ...
     */
    public static boolean isRadioOn(PhoneAccountHandle account) {
        int subId = Integer.parseInt(account.getId());
        Log.d(TAG, "[isRadioOn]subId:" + account.getId());
    ...
    public static boolean isSupport3GVT() {
        return android.os.SystemProperties.get("ro.mtk_vt3g324m_support").equals("1");
    }
}
