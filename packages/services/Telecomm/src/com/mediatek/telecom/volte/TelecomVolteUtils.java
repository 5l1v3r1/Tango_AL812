...
//----------------------------For volte ims call only------------------------------------
    ...
    public static String updateDisplayNameFromPau(Bundle bundle, String defaultDisplayName) {
        String displayName = defaultDisplayName;
        if (bundle == null || !bundle.containsKey(TelecomManagerEx.EXTRA_VOLTE_PAU_FIELD)) {
            log("updateDisplayNameFromPau()... bundle is null, or not contain pau field.");
        } else {
            String pauField = bundle.getString(TelecomManagerEx.EXTRA_VOLTE_PAU_FIELD,"");
            displayName = getNameFromPAU(pauField);
            log("Display-name changed: " + defaultDisplayName + " -> " + displayName);
        }
        return displayName;
    }
