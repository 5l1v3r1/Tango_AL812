...
public class MobileNetworkSettings extends PreferenceActivity implements
	...
	/*HQ_xionghaifeng 20160323 add for HQ01825477 start*/
    public static final int SVLTE_RAT_MODE_4G = 0;
    public static final int SVLTE_RAT_MODE_3G = 1;
	/*HQ_xionghaifeng 20160323 add for HQ01825477 end*/
        ...
        switch (state) {
            case UPDATE: {
                ...
                for (int index = 0; index  < mActiveSubInfos.size(); index++) {
                    ...
                    // xdf add
                    //final SubscriptionInfo sir = findRecordBySlotId(index); 
                    //log("xdf add for slotid", " sir=" + sir); 
                    // xdf end
    ...
    protected void onCreate(Bundle icicle) {
		...
		//add by wanghui  for al812 20150921,HQ01393161
    ...
    private void updateBody() {
        ...
        if (prefSet != null) {
			...
			if(!isTlVersion()){//HQ_liugang add for HQ01754573
    ...
    private void updateEnhanced4GLteSwitchPreference() {
		//modify by HQ_sunsongjian for the HQ01834652
        if(TelephonyUtils.isCmccCard(mPhone.getSubId()) 
			|| TelephonyUtils.isHwLabCard(mPhone.getSubId())){
		...
		if (SubscriptionManager.isValidSubscriptionId(mSubId))
		{
			...
			if (phoneType == PhoneConstants.PHONE_TYPE_CDMA)
			{
				...
				Log.i("xionghaifeng-net","ratMode = "+ratMode);

				if (ratMode != SVLTE_RAT_MODE_3G)
				{
					lteServicesIsOn = true;
				}
				else
				{
					lteServicesIsOn = false;
				}
			}
			else
			{//GSM phone
				...
				current_lte_mode = Settings.Global.getInt(mPhone.getContext().getContentResolver(),
					Settings.Global.PREFERRED_NETWORK_MODE + mSubId, Phone.PREFERRED_NT_MODE);
				Log.i("xionghaifeng-net","current_lte_mode = "+current_lte_mode);
				
				if (current_lte_mode == Phone.NT_MODE_LTE_CDMA_AND_EVDO
					|| current_lte_mode == Phone.NT_MODE_LTE_GSM_WCDMA
					|| current_lte_mode == Phone.NT_MODE_LTE_CDMA_EVDO_GSM_WCDMA
					|| current_lte_mode == Phone.NT_MODE_LTE_ONLY
					|| current_lte_mode == Phone.NT_MODE_LTE_WCDMA)
				{
					lteServicesIsOn = true;
				}
				else
		...
		//modify by zhangjinqiang for HQ02069847-start
		if(mEnhancedButton4glte != null){
			if (lteServicesIsOn)
			{
		...
		//modify by zhangjinqiang for HQ02069847-end
		/*HQ_xionghaifeng 20160323 add for HQ01825477 end*/
    }
	...
	/*HQ_liugang add for HQ01754573*/
	private boolean isTlVersion(){
		String productName = SystemProperties.get("ro.product.name", "");
		return "TAG-TL00".equalsIgnoreCase(productName);
	}
	/*HQ_liugang add end*/
}
