...
public class MobileNetworkSettingsEx extends Activity{
	...
	private static final int TD_MODEM_ON = 0;
  ...
  @Override
	...
	private void setLteServiceAbility(boolean isChecked, int subId)
	{
		...
		{
			android.provider.Settings.Global.putInt(mPhone.getContext().getContentResolver(),
				android.provider.Settings.Global.PREFERRED_NETWORK_MODE, TD_MODEM_ON);
			//add by liruihong
			android.provider.Settings.Global.putInt(mPhone.getContext().getContentResolver(),
			      android.provider.Settings.Global.PREFERRED_NETWORK_MODE + subId , TD_MODEM_ON);
			mPhone.setPreferredNetworkType(TD_MODEM_ON, null);
		}	
