...
public class PhoneAccountSettingsFragment extends PreferenceFragment
    ...
    public void onResume() {
		...
		smartIdentifyNumber = (PreferenceScreen)findPreference("identify_number");
		int identifyValue = Settings.System.getInt(getActivity().getApplicationContext().getContentResolver(), "identify_switch", 0);					
		if(identifyValue==1){
			smartIdentifyNumber.setSummary(R.string.identify_enable);
		}else{
			smartIdentifyNumber.setSummary(R.string.identify_disable);
		}
		smartIdentifyNumber.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			
			@Override
			public boolean onPreferenceClick(Preference arg0) {
				// TODO Auto-generated method stub
				
				Intent intent = new Intent(getActivity(),SmartPhoneIdentifyActivity.class);
    ...
    private void updateDefaultOutgoingAccountPreference() {
        if (mDefaultOutgoingAccount != null) {
            ...
            if (mayCauseSimSwitch) {
                boolean isAirplaneModeEnabled = TelephonyUtils.isAirplaneModeOn(PhoneGlobals.getInstance());
