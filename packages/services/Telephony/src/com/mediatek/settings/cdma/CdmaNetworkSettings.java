...
public class CdmaNetworkSettings {
    ...
    private void updateSwitch() {
        ...
        //HQ_sunliang 20160201 add for HQ01669945 start
        mDataOnlyPreference.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                if (preference.getKey().equals(SINGLE_LTE_DATA)) {
                     boolean checked = ((Boolean) newValue).booleanValue();
                     mDataOnlyPreference.setChecked(checked);
        ...
        //HQ_sunliang 20160201 add for HQ01669945 end
    ...
    private void handleEnable4GDataClickForCT(Preference preference) {
        ...
        int networkType = isChecked ?
                (SvlteRatController.RAT_MODE_SVLTE_2G
                 ...
                 : (SvlteRatController.RAT_MODE_SVLTE_2G
                    | SvlteRatController.RAT_MODE_SVLTE_3G);
        Log.d(TAG, "CT 6M isChecked = " + isChecked + " ratMode = " + ratMode);
