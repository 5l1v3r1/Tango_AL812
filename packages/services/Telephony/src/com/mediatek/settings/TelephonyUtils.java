...
public class TelephonyUtils {
    ...
    private static final String[] HW_LAB_NUMERIC = {"46070"};//add by sunsongjian
	...
	//add by sunsongjian begin
	public static boolean isHwLabCard(int subId) {
        boolean result = false;
        String numeric = TelephonyManager.getDefault().getSimOperator(subId);
        for (String mccmnc : HW_LAB_NUMERIC) {
            if (mccmnc.equals(numeric)) {
                result = true;
            }
        }
        log("isHwLabCard:" + result);
        return result;
    }
	//add end
