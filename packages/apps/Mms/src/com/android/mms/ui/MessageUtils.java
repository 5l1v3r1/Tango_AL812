...
//        final long totalBlocks = stat.getBlockCount();
  ...
  //add by lipeng for al813 mms
    ...
    /*get sim card mnc mcc without Virtual*/
    public static String getDefaultMncMcc(){
       String simMNCMCC1 = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC);
       String simMNCMCC2 = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC_2);
               
               String codeMNCMCC = "999999";
               if(simMNCMCC1!=null && simMNCMCC1.length()>=5){
                       codeMNCMCC = simMNCMCC1;
               }else if(simMNCMCC2!=null && simMNCMCC2.length()>=5){
                       codeMNCMCC = simMNCMCC2;
               }
               Log.d(TAG, "MMS simcard mncmcc: "+codeMNCMCC);
               return codeMNCMCC;
    }
   ...
   public static String getSimMccMnc(int slotId) {
    ...
    /*get sim card mnc mcc with Virtual
    public static String getDefaultMncMcc(){
    	String simMNCMCC1 = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC);
    	String simMNCMCC2 = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC_2);
		...
		Log.d(TAG, "MMS simcard mncmcc start: "+codeMNCMCC);
		//Log.d(TAG, "MMS simcard getDefaultSMSSlotId(): " + getDefaultSMSSlotId());

        int mSlotId = MmsConfig.getDefaultSMSSlotId();
        ArrayList<String> simType;
        Log.d(TAG, "MMS simcard mSlotId: "+mSlotId);
        if(mSlotId!=-1){
        	simType = PhoneNumberUtils.getVirtualType(mSlotId);
        	Log.d(TAG, "MMS simcard mncmcc dul : "+simType.get(0)+"  "+simType.get(1));
        	...
        	if(simMNCMCC2!=null && simMNCMCC2.length()>=5){
            	simType = PhoneNumberUtils.getVirtualType(1);
            	Log.d(TAG, "MMS simcard mncmcc sim2: "+simType.get(0)+"  "+simType.get(1));
            	if(simType.get(0).equals("-1")){
            		return simType.get(2);
            	}
                return simType.get(1);
        	...
        	if(simMNCMCC1!=null && simMNCMCC1.length()>=5){
            	simType = PhoneNumberUtils.getVirtualType(0);
            	Log.d(TAG, "MMS simcard mncmcc sim1: "+simType.get(0)+"  "+simType.get(1));
            	if(simType.get(0).equals("-1")){
            		return simType.get(2);
            	}
                return simType.get(1);
    ...
    public static int loadMmsSizeByMncMcc(String keyword) {
    	...
    	Log.d(TAG, "MMS loadMmsSizeByMncMcc: "+size);
        return size;
    }
    ...
    public static void resetEncodeType(Context context){
     ...
     /*add by lipeng for 7bit get sim card mnc mcc*/
    public static String getDefaultMncMccFor7Bit(){
		String simMNCMCC1 = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC);
		String simMNCMCC2 = SystemProperties.get(TelephonyProperties.PROPERTY_ICC_OPERATOR_NUMERIC_2);
    ...
    public static boolean loadCan7BitChangeByMncMcc(String keyword) {
    	...
    	if(mSevenConfigOverride.containsMccmncForSevenBit(keyword)){
    		...
    		//Log.d(TAG,"match result:"+key); 
    		if("1".equals(key)){
    			canClick = true;
    		}
    	}
    	//Log.d(TAG, "MMS loadCan7BitChangeByMncMcc: "+canClick);
        return canClick;
    } 
