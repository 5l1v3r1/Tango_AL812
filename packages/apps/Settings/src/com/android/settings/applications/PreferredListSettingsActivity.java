...
public class PreferredListSettingsActivity extends PreferenceActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (SmsApplication.getSmsApplicationCollection(mContext).size() == 0) {
            Log.d(TAG, "will remove messageScreen");
            getPreferenceScreen().removePreference(messageScreen);
        }
    }
    ...
    public String getPreferredSummary(String key) {
        ...
        if (key.equals(LISTPREF_HOME_KEY)) {
            ...
            Log.d(TAG, "defaultComponent: " + defaultComponent);
            String smsPreferredSummary = "";
            String packageName = null;
            if (defaultComponent != null) {
                 packageName = defaultComponent.getPackageName();
            } else {
                List<ResolveInfo> mList = SmsApplication.getSmsApplicationCollection(mContext);
                if (mList != null && mList.size() > 0) {
                    ResolveInfo first = mList.get(0);
                    packageName = first.activityInfo.applicationInfo.packageName;
                }
            }
            try {
        ...
        List<ResolveInfo> mList = null;
        //we should special deal with home exclude get_disabled_components.
        if (key.equals(LISTPREF_HOME_KEY)) {
            mList = getPackageManager().queryIntentActivities(startIntent, (PackageManager.GET_INTENT_FILTERS
