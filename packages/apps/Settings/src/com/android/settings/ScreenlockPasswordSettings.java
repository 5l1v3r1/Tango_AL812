...
import com.android.settings.fingerprint.FingerprintEnterActivity;
import com.android.settings.fingerprint.FingerprintManageActivity;
...
public class ScreenlockPasswordSettings extends SettingsPreferenceFragment implements OnPreferenceChangeListener{
	...
	private static final int RESULT_FINISHED = 1;//add by liuchaochao for HQ01401884
	Uri uri = Uri.parse("content://com.huaqin.fingerprintname.FingerprintNameProvider");//add by liuchaochao for HQ01401884
	...
	//add by liuchaochao for HQ01401884 start
	@Override
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        Log.e(TAG, "onActivityResult requestCode=" + requestCode + " , resultCode= " + resultCode );
        if (requestCode == SET_OR_CHANGE_LOCK_METHOD_REQUEST && resultCode == RESULT_FINISHED) {
            if (isFingerprintExist()) {
                dialogInvitetoEnrol();
            } else {// add by liuchaochao for HQ01401891
                if (!isFingerprintLockAllowed()) {
                    associateFpdialog();
                }
            }
        }
    ...
    private boolean isFingerprintExist() {
        ...
        try {
            ...
            Log.e(TAG, "isFingerprintExist cursor.getCount() = " + cursor.getCount());
        ...
        builder.setNegativeButton((R.string.fingerprint_enrol_now),
                new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        intent1.putExtra("safe_enter", "fingerprint_enter");
    ...
    //add by liuchaochao for HQ01401884 end

    //add by liuchaochao for HQ01401891 start
    ...
    protected void associateFpdialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
        builder.setTitle(R.string.associate_fingerprint_title);
        builder.setMessage(getAssociateFpMsg());
        builder.setPositiveButton((R.string.associate_fingerprint_yes),
                new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
                        setFingerprintLockAllowed(true);
                        Intent intent1 = new Intent(getActivity(), FingerprintManageActivity.class);
                        intent1.putExtra("safe_enter", "fingerprint_manager");
                        intent1.putExtra("associate_fp", true);
                        getActivity().startActivity(intent1);
                    }
                });
        builder.setNegativeButton((R.string.associate_fingerprint_cancel),
                new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
    ...
    private int getAssociateFpMsg() {
        int msg = R.string.associate_fingerprint_pin_msg;
        switch (mLockPatternUtils.getKeyguardStoredPasswordQuality()) {
            case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC:
            case DevicePolicyManager.PASSWORD_QUALITY_NUMERIC_COMPLEX:
                msg = R.string.associate_fingerprint_pin_msg;
                break;
            case DevicePolicyManager.PASSWORD_QUALITY_ALPHABETIC:
            case DevicePolicyManager.PASSWORD_QUALITY_ALPHANUMERIC:
            case DevicePolicyManager.PASSWORD_QUALITY_COMPLEX:
                msg = R.string.associate_fingerprint_password_msg;
                break;
        }
        return msg;
    }
    //add by liuchaochao for HQ01401891 end
    ...
    private boolean isLockEnabledAllowed() {
    	...
    	//add by HQ_caoxuhao at 20150916 HQ01377878 begin
        String unlockSetOrChangeSummary = mUnlockSetOrChange.getSummary().toString();
        String unlockModeOff = getActivity().getResources().getString(R.string.unlock_set_unlock_mode_off);
        if (unlockSetOrChangeSummary.equals(unlockModeOff)) {
			return false;
		}
        
        return true;
      //add by HQ_caoxuhao at 20150916 HQ01377878 end
    }
