...
import com.android.internal.widget.LockPatternUtils;
import android.app.admin.DevicePolicyManager;
public class FingerprintComplexPassword extends Fragment{

	private LockPatternUtils mLockPatternUtils;
	private static final String TRUST_AGENT_CLICK_INTENT = "trust_agent_click_intent";
	private final String TAG = "FingerprintComplexPasswords";
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView =  inflater.inflate(R.layout.fingerprint_complex_password, container,false);
		return rootView;
	}
	@Override  
	public void onActivityCreated(Bundle savedInstanceState) {  
		...
		TextWatcher textWatcher = new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				String noteText;
				if(addpasswod == 0){
                    if (s.toString().length() > 0) {//add by liuchaochao for HQ01401883&HQ01401881
                        if (s.toString().length() < 4) {
                            noteText = getString(R.string.complex_password_demand);
                            mGuideText.setText(noteText);
                            mNextstep.setClickable(false);
                            mNextstep.setTextColor(Color.GRAY);
                            mNextstep.setBackgroundResource(R.drawable.button_border_unclickable);
                        } else {
                            noteText = validatePassword(s.toString());
                            if (noteText != null) {
                                mGuideText.setText(noteText);
                                mNextstep.setClickable(false);
                                mNextstep.setTextColor(Color.GRAY);
						...
						noteText = getString(R.string.enter_complex_password);
						mGuideText.setText(noteText);
					}
				}
				else{
				...
				if(mGuideText.getText().equals(getString(R.string.click_continue))){
					...
					if(mPassword.getText().toString().equals(Password)){ 
						...
						mLockPatternUtils = new LockPatternUtils(getActivity());
  						...
  						mLockPatternUtils.saveLockPassword(Password, 262144, false,  null);
    ...
    private String validatePassword(String password) {
        if (password.length() < 4) {
            return getString(R.string.complex_password_demand);
        }
        if (password.length() > 16) {
            return getString(R.string.lockpassword_password_too_long, 17);
        }
        int letters = 0;
