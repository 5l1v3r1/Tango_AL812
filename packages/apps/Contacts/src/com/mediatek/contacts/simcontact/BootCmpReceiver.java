...
public class BootCmpReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(TelephonyIntents.ACTION_PHB_STATE_CHANGED)) {			
            ...
            //add by tanghuaizhe  for huawei hotline start
            huaweiPreference=context.getSharedPreferences("hotLine",context.MODE_PRIVATE);
            SharedPreferences.Editor huaweiEditor=huaweiPreference.edit();
            //删除之后不再预置hotlien
            if(!huaweiPreference.getBoolean("deleted",false))
            {
...
/**         需要和ota方面确认，升级成功后恢复hotline
            File mUpdateResultDir = new File("/data/system");
            if (!mUpdateResultDir.exists()) {
                mUpdateResultDir.mkdirs();
            }
            String fileName = "updateResult.xml";
            File file = new File(mUpdateResultDir, fileName);
            if (!file.exists()) {
//                升级成功，执行预制联系人的操作
                startSimService(context, -1, SIMServiceUtils.SERVICE_WORK_IMPORT_PRESET_CONTACTS);
            }
