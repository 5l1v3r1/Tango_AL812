 ...
 */
public class ExpandingEntryCardView extends CardView implements IServiceStateCallback {
    ...
    private View createEntryView(LayoutInflater layoutInflater, final Entry entry,
		...
		if(entry.getAlternateIcon()!=null){
            ...
            if (isCallLog) {
			    ...
			    view = (EntryView)layoutInflater.inflate(
                    R.layout.expanding_entry_call_card_item_hw, this, false);
            }
        ...
        if (!TextUtils.isEmpty(entry.getText())) {
            ...
            if (hour_index != -1 && min_index != -1) {
                if (!DateFormat.is24HourFormat(getContext())) {
                    int value = Integer.parseInt(date.substring(hour_index + 1 , min_index));
                    String head = date.substring(0 , hour_index + 1);
                    String tail = date.substring(min_index);
                    String time_mark = "";
                    if (value > 12) {
                        value -= 12;
                        time_mark = getContext().getString(R.string.pm_time);
                    } else if (value == 0) {
                        value += 12;
                        time_mark = getContext().getString(R.string.am_time);
                    } else {
                        time_mark = getContext().getString(R.string.am_time);
                    }
                    tail = value + tail;
...
//add by zhangjinqiang for al812 call duration --start
	private String formatLongTime(String stringTime){
		//modify by wangmingyue for HQ01524104 begin
		int time=0;
		try {
		...
		//modify by wangmingyue for HQ01524104 end
		//int hour = time/3600;
