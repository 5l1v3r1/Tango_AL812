...
//	private RelativeLayout MyDock;
	...
	//是否保存在SIM卡中的联系人
 	private boolean isSimContact(Uri uri) {
		...
		try {
			...
			while (dataCursor.moveToNext()) {
				indicate = dataCursor.getString(0);
				LogUtils.d(TAG, "indicate = " + indicate);
			}
			if (!indicate.equals("-1")) {
				return true;
			} else {
				return false;
			}
	...
	private void buildContextMenu(final int position) {
		...
		if (uri != null && !isSimContact(uri)) {
			final String[] arrayMenuItems = getResources().getStringArray(R.array.phone_contact_menu);
			if (isStarred(uri)) {
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
		...
		String contactstoDisplay = getResources().getString(
			...
			public void onTouchingLetterChanged(String s) {
				...
				for (int i = 0; i < Sections.length; i++) {
					...
					if (TouchCharNum == Sections[i].charAt(0)) {
                        ...
                        /* modify by shanlan for HQ01930907 begin */
                        //getListView().setSelection(position);
                        int count = getAdapter().getCount();
                        if (position < count) {
                            getListView().setSelection(position + 3);
                        }
                        /* modify by shanlan for HQ01930907 end */
					}
					// Log.i(TAG,
