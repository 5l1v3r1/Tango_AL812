...
public class GroupBrowseListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(language.equals("en")) {
		    if(entry.getTitle().equals("同事")) {
		        viewCache.groupTitle.setText("Work");
		    } else if(entry.getTitle().equals("同学")) {
		        viewCache.groupTitle.setText("Classmates");
		    } else if(entry.getTitle().equals("家人")) {
		        viewCache.groupTitle.setText("Family");
		    } else if(entry.getTitle().equals("朋友")) {
		        viewCache.groupTitle.setText("Friends");
		    } else {
		        viewCache.groupTitle.setText(entry.getTitle());
		    }
		} else if (language.equals("zh") && (country.equals("TW") || country.equals("HK"))) {
		    if(entry.getTitle().equals("同事")) {
		        viewCache.groupTitle.setText("同事");
		    } else if(entry.getTitle().equals("同学")) {
		        viewCache.groupTitle.setText("同學");
		    } else if(entry.getTitle().equals("家人")) {
		        viewCache.groupTitle.setText("家人");
		    } else if(entry.getTitle().equals("朋友")) {
		        viewCache.groupTitle.setText("朋友");
		    } else {
		        viewCache.groupTitle.setText(entry.getTitle());
		    }
		} else {
	...
	private void bindHeaderView(GroupListItem entry,
		...
		String account_type = accountType.accountType;
                ...
                if(slot==0)accountTypeString = mContext.getResources().getString(R.string.hq_sim_slot_position_1);
		if(slot==1)accountTypeString = mContext.getResources().getString(R.string.hq_sim_slot_position_2);
   	        //added by zhenghao , HQ01320081 end 

                /*removed by zhenghao because of HQ01320081
		//add by tanghuaizhe for huawei ui start
		if(accountName!=null&&accountTypeString!=null){
			if (accountName.equals("USIM1")) {
				accountTypeString = accountTypeString + " 1";
			} else if (accountName.equals("USIM2")) {
				accountTypeString = accountTypeString + " 2";
			}
		}
