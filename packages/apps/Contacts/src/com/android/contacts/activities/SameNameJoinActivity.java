...
//						AccountType.ACCOUNT_TYPE_USIM }, Contacts.DISPLAY_NAME);
		...
		if(queryContactsTask != null)//HQ_liugang add for HQ01769224
...
//		}
	...
	private final class QueryHandler extends AsyncQueryHandler {
		...
		protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
			...
			if (cursor == null || cursor.getCount() == 0) {
				Toast.makeText(SameNameJoinActivity.this, "查询不到联系人！",
	...
	private class QueryContactsTask extends AsyncTask<Void, Cursor, Cursor> {
		...
		protected Cursor doInBackground(Void... params) {
			...
			if(currentFilterAccountName!=null){//不对sim卡联系人进行合并操作
				cursor = db
						.rawQuery(
								"select _id,display_name from view_raw_contacts where ((account_type!=? and account_type!=? and account_type!=? and account_type!=? ) or account_type is null) and (deleted!=?) and account_name =? order by  display_name ",
								new String[]{"USIM Account","SIM Account","CSIM Account","com.tencent.mm.account","1",currentFilterAccountName});
				
			}else {
				cursor = db
						.rawQuery(
								"select _id,display_name from view_raw_contacts where ((account_type!=? and account_type!=? and account_type!=? and account_type!=?  ) or account_type is null) and (deleted!=?) order by  display_name ",
								new String[]{"USIM Account","SIM Account","CSIM Account","com.tencent.mm.account","1"});
