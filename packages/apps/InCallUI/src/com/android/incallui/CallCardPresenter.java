...
public class CallCardPresenter extends Presenter<CallCardPresenter.CallCardUi>
    ...
    public void init(Context context, Call call) {
				//add by zhangjinqiang for al812--start
		if(csom==null){
			InCallActivity inCallActivity = (InCallActivity)context;
...
/**add by liruihong for HQ01440479*/
    ...
    private  String getNameForCallHW(ContactCacheEntry contactInfo) {
        CallerIdResult  mCallResult=null;
        if(!TextUtils.isEmpty(contactInfo.name)){
			return contactInfo.name;
		}
		int isConnected = Settings.System.getInt(mContext.getContentResolver(), "isConnected", 0);
		
		if(csom!=null&&isConnected!=0){
			mCallResult = csom.getCallerIdResult(contactInfo.number,CallerIdStrategy.OFFLINE_ONLY);
		}
	...
	private  String getLocationForCallHW(ContactCacheEntry contactInfo){
		...
		String number = formatNumber(contactInfo.number);

		int isConnected = Settings.System.getInt(mContext.getContentResolver(), "isConnected", 0);
		if(csom!=null&&isConnected!=0){
			return csom.getPhoneAttribute(number);
		}else{
			return "";
		}
	}
    ...
    public interface CallCardUi extends Ui {
        ...
        */
		...
		void setPrimaryPhoneLocator(String location);
        /// @}
    }
	...
	//add by zhangjinqiang for match phone number-starts
	...
	public void setLocation(String number){
		...
		if(formatNumber!=null&&csom!=null){
			location =  csom.getPhoneAttribute(formatNumber);
		}
		getUi().setPrimaryPhoneLocator(location);
	}
