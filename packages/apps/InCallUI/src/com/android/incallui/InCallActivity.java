...
public class InCallActivity extends Activity implements IServiceStateCallback{
    ...
    /// @}
    NetWorkStateChanageReceiver NTSChanageReceiver;
	...
	if(InCallApp.gIsHwUi){
		...
		NTSChanageReceiver = new NetWorkStateChanageReceiver();
		IntentFilter intentFilter = new IntentFilter();
		intentFilter.addAction("huawei.intent.action.IMS_SERVICE_STATE_CHANGED");
		registerReceiver(NTSChanageReceiver, intentFilter);
		Log.d(this, "onCreate(): exit");
		//add by zhangjinqiang for HQ01894022 -start
		disableSystemBarExpand(true);
		//add by zhangjinqiang for HQ01894022 -end
    }
    	class NetWorkStateChanageReceiver extends BroadcastReceiver {
		   public void onReceive(Context context, Intent intent) {
			//int reg = intent.getIntExtra(ImsManager.EXTRA_IMS_REG_STATE_KEY , -1);
			String state = intent.getStringExtra("state");
			Log.i(this,"onReceive ACTION_SUBINFO_CONTENT_CHANGE "+ state);
			if(state == null){
				return;
			}
			if(state.equals("UNREGISTERED")){
				mCallCardFragment.setNtStatusInvisible();
			}
		   }
	}
	private Drawable getOneScreenBitmap() {
		...
		android.util.Log.d("getOneScreenBitmap", "bitmap Drawable width="
				+ width + ",high=" + height);
		/* HQ_fengsimin 2016-3-24 modified for HQ01824243 */
		if((width<height&&actualWidth>actualHeight)||(width>height&&actualWidth<actualHeight)){
    ...
    protected void onResume() {
        Log.i(this, "onResume()...");
        super.onResume();
        /* HQ_xuqian4 20151126 modified for HQ01527075 begin */
        
        disableSystemBarExpand(false);
        /* HQ_xuqian4 20151126 modified end*/
	...
	unregisterReceiver(NTSChanageReceiver);//add by qiyanlong
        super.onDestroy();
        ...
        csom = null;
        /* end:change by donghongjing for HQ01442778 */
		Settings.System.putInt(getContentResolver(), "isConnected", 0);
	//add by zhangjinqiang for HQ01894022-start
	disableSystemBarExpand(false);
	//add by zhangjinqiang for HQ01894022-start
...
//end
	...
	public void onServiceConnected() {
		//add by zhangjinqiang for display incoming call location-start
		CallList callList = CallList.getInstance();
		if(callList!=null&&callList.getFirstCall()!=null){
			String number = callList.getFirstCall().getNumber();
			mCallCardFragment.getPresenter().setLocation(number);
		}
		Settings.System.putInt(getContentResolver(), "isConnected", 1);
		//add by zjq end
	}
