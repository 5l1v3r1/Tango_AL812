...
public class ContactListFilterView extends LinearLayout {
	...
	private static final String DATA_DATA_COM_ANDROID_PROVIDERS_CONTACTS_DATABASES_CONTACTS2_DB = "/data/data/com.android.providers.contacts/databases/contacts2.db";
...
//	private ImageView mIcon;
	...
	public ContactListFilterView(Context context) {
		...
		if (db==null||!db.isOpen()) {
			db = SQLiteDatabase
					.openDatabase(
							DATA_DATA_COM_ANDROID_PROVIDERS_CONTACTS_DATABASES_CONTACTS2_DB,
							null, 0);
		}
	}
	...
	public static void closeContactsDb() {
		if(db!=null){
			if(db.isOpen()){
				db.close();
			}
		...
		new Thread() {
			public void run() {
				...
				if(mFilter.accountName.equals("Phone")){
					...
					cursor = db
							.rawQuery(
									"select distinct contact_id  from view_raw_contacts  where account_name=?",
									new String[] { mFilter.accountName});
				}
				if (cursor != null) {
					//i = cursor.getCount();
					//add by HQ_xiatao 20150902 HQ01354327 start
					...
					//add by HQ_xiatao 20150902 HQ01354327 end
					cursor.close();
				} else {
					i = 0;
				}
